...
"""
    ...
    def cmdlinestart(self):
        ...
        try:
            ...
            # Start the server
            self.start(host=host, port=port, replicas=replicas,
    ...
    def add_new_node(self, new_node):
        ...
        ring = map(str, self.hash_ring.get_nodelist())
            ...
            for server in key_is_at:
                ...
                if remote_time < local_timestamp:
                    ...
                    value = self.dht_table.perform(
                        DHTCommand(DHTCommand.GET, key))["value"]
                    command = DHTCommand(
                        DHTCommand.PUT, key, value, local_timestamp)
                    status = self.client.sendcommand(server, command)["status"]
                    ...
                    value = self.client.sendcommand(server, command)["value"]
                    status = self.dht_table.perform(DHTCommand(
                        DHTCommand.PUT, key, value, remote_time))["status"]
    ...
    def handle_replica_command(self, command, client_sock):
        ...
        replica_servers = " ".join(map(str, key_is_at))
        ...
        logging.debug("%s is at [%s] according to [%s], is local(%s)", command.key, replica_servers, str(self.hash_ring), str(local))
        # If key is on this server, perform the action, if it was a get return immediately
        if local:
            status = self.dht_table.perform(command)
            logging.debug("local status: %s", status)
            if command.action == DHTCommand.GET and status["success"] == True:
                return status
        ...
        if not command.forwarded:
            ...
            command.forwarded = True
            for server in key_is_at:
                response = self.client.sendcommand(
                    server, copy.deepcopy(command))
    ...
    def server_thread(self, client_sock):
        ...
        rawcommand = self.client.read_from_socket(client_sock)
    ...
    def initialise_hashring(self):
        ...
        if self.remote_server:
            ...
            nodes = map(lambda server_string: Server(server_string.split(
                ":")[0], server_string.split(":")[1]), nodes)
