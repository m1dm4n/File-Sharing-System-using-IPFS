...
class HashRing(object):
    ...
    def add_node(self, node):
        ...
        for i in range(self.distribution_points):
            key = self.gen_key(f"{node}:{i}".encode())
            self.ring[key] = node
    ...
    def gen_key(self, key: bytes):
        ...
        """
        return int.from_bytes(Hashing(key), 'big')
